...
1. 📋 **Registro de Excepción de Almacenamiento (O-11):** [`docs/EXCEPCION_ALMACENAMIENTO_OCI.md`](EXCEPCION_ALMACENAMIENTO_OCI.md)  
...
| **O-03** | Orquestación con LLM (Google Gemini) | 🟡 **PENDIENTE** | **Cliente migrado a `google-genai` listo en [`src/llm/engine.py`](../src/llm/engine.py); ejecución real pendiente de provisión de `GEMINI_API_KEY` por el squad.** Salida referencial en [`docs/contratos_referencia/ejemplo_gemini_google_genai.json`](contratos_referencia/ejemplo_gemini_google_genai.json). |
| **O-04** | Pipeline RAG: segmentación (chunking), embeddings y Vector Store | 🟢 **VERIFICADO** | [`src/ingestion/chunker.py`](../src/ingestion/chunker.py) (1000/150 referencial y configurable) y [`src/rag/vector_store.py`](../src/rag/vector_store.py) (ChromaDB + `all-MiniLM-L6-v2`). Verificado en [`tests/test_rag_pipeline.py`](../tests/test_rag_pipeline.py). |
| **O-05** | Adaptación del mismo documento para al menos 2 perfiles y 2 formatos | 🟢 **VERIFICADO** | Vinculado directamente a los contratos sobre arquitectura VCN OCI:<br>• **Contrato 01:** [`docs/contratos_referencia/ejemplo_01_vcn_principiante_flashcards.json`](contratos_referencia/ejemplo_01_vcn_principiante_flashcards.json) (Perfil Principiante / Formato Flashcards)<br>• **Contrato 02:** [`docs/contratos_referencia/ejemplo_02_vcn_arquitecto_tutorial.json`](contratos_referencia/ejemplo_02_vcn_arquitecto_tutorial.json) (Perfil Arquitecto / Formato Tutorial). |
...
| **O-11** | Almacenamiento de contenidos generados en OCI Object Storage | 🟠 **EXCEPCIÓN** | Documentada y certificada en [`docs/EXCEPCION_ALMACENAMIENTO_OCI.md`](EXCEPCION_ALMACENAMIENTO_OCI.md). Adaptador conmutable con prueba y salida archivada en [`tests/test_oci_storage.py`](../tests/test_oci_storage.py). |
| **O-12** | Mínimo de tres ejemplos de ejecución documentados | 🟢 **VERIFICADO** | Declarados y versionados en [`docs/contratos_referencia/`](contratos_referencia/):<br>1. [`ejemplo_01_vcn_principiante_flashcards.json`](contratos_referencia/ejemplo_01_vcn_principiante_flashcards.json)<br>2. [`ejemplo_02_vcn_arquitecto_tutorial.json`](contratos_referencia/ejemplo_02_vcn_arquitecto_tutorial.json)<br>3. [`ejemplo_03_seguridad_ejecutivo_resumen.json`](contratos_referencia/ejemplo_03_seguridad_ejecutivo_resumen.json). |
| **O-13** | Interfaz de usuario con los cuatro parámetros de control requeridos | 🟢 **VERIFICADO** | [`ui/app.py`](../ui/app.py) (Selector de Perfil, Formato, 10 Sectores Canónicos y Nivel de Detalle). |
| **O-14** | Suite de pruebas automatizadas que valide el flujo completo | 🟢 **VERIFICADO** | Suite automatizada en [`tests/`](../tests/) ejecutada con Pytest (52/52 passing). |
...
4. **Calibración de Anclaje a la Fuente y Pendiente para Squad 1:** Para textos cortos, la métrica penalizaba la expansión explicativa pedagógica. Se recalibró `src/quality/evaluator.py` con stopwords técnicas y ponderación. **Advertencia para Squad 1:** La fórmula actual establece un piso de 0,85 por diseño y mide cobertura sobre ese piso; queda como pendiente técnica obligatoria dotar al evaluador de capacidad punitiva perceptible (reprobación < 0,70) ante desvíos o alucinaciones deliberadas, ya que *un indicador que no puede reprobar no discrimina* ([`docs/DECISION_TECNICA_CHUNKING.md`](DECISION_TECNICA_CHUNKING.md)).
5. **Inercia de CPU en Documentos Masivos y Particionado RAG:** La indexación exhaustiva en CPU de un documento técnico extenso (~965.000 caracteres / ~1.300 fragmentos) demoraba hasta 5 minutos para producir únicamente 4 tarjetas pedagógicas. Se implementó una política de lote representativo inicial de 80 fragmentos (`MAX_CHUNKS_PER_BATCH = 80`), alerta preventiva de latencia en Paso 1/3, progreso reactivo con conteo real de fragmentos y botón '➕ Lote Adicional', reduciendo el tiempo a ~12-15 segundos (mejora del 96% en latencia).
